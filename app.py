...
# =========================
ports_file = st.file_uploader("Upload your ports.csv (must contain Name, Latitude, Longitude columns)", type=["csv"])
if not ports_file:
...
try:
    ...
    st.error("Python package `searoute` is not available. Please install it on the server: `pip install searoute`.")
    st.stop()
...
with st.sidebar:
    st.header("Inputs")
    vessel_name = st.selectbox("Vessel", list(VESSELS.keys()), index=list(VESSELS.keys()).index("Up-to 7000 DWT") if "Up-to 7000 DWT" in VESSELS else 0)
...
ports = PortOps(
    ...
    port_fees_usd=port_fees, canal_fees_usd=canal_fees, other_costs_usd=other_costs, misc_costs_usd=misc_costs
)
legs: List[LegByName] = []
...
with tab1:
    ...
    if st.button("Calculate TCE", type="primary"):
        ...
        st.dataframe(pd.DataFrame(res["per_leg"]))
